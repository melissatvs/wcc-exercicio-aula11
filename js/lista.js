class Lista {
   ...
   montaLista = async () => {
      ...
      this.arrayDeItens.forEach(item => {
         conteudo += `<li id="${item.id}">${item.name} <button class="botao deletar">DEL</button></li>`
      })
   ...
   deletarItemDaLista = async (event) => {
      const itemDeletado = event.target.previousSibling.parentElement
      ...
      if (event.target.classList.contains("deletar")) {
         ...
         if (confirm("Deseja realmente excluir?")) {
            ...
            await fetch(this.url + "/" + itemDeletado.id, requisicao)
              .then(resposta => resposta.json())
              .then(dados => this.arrayDeItens = dados)
         }
      }
